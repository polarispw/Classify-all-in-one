...
from data.processors import processors_mapping, median_mapping
...
class MyDataset(Dataset):
    ...
    def __init__(self, args, tokenizer, cache_dir=None, mode="train", use_demo=False):
        ...
        assert mode in ["train", "dev", "test"], "Only support train, dev, and test mode"
        ...
        if args.prompt and args.mapping is not None:
            ...
            for key in self.label_to_word:
                ...
                logger.info(
                    "Label {} to word {} ({})".format(key, tokenizer.convert_ids_to_tokens(self.label_to_word[key]),
        ...
        # Load cache: Cache name distinguishes mode, task name, tokenizer, and length. So if you change anything
        # beyond these elements, make sure to clear your cache.
    ...
    def select_context(self, context_examples):
        ...
        if len(self.label_list) == 1:
            # Regression task, only calculate a scalar
            counts = {'0': 0, '1': 0}
        selection = []
        ...
        if self.args.gpt3_in_context_head or self.args.gpt3_in_context_tail:
            ...
            assert len(selection) > 0, "Not enough selections"
    ...
    def convert_fn(
        ...
        if not use_demo:
            ...
            # is the sentence 0; for sentence-pair tasks, the input(query) is the sentence 0 and 1. Note that for GPT-3
            ...
            if sfc_template is not None:
                ...
                sfc_feature = self.convert_fn(
                    example, supports, use_demo=use_demo, label_list=label_list, prompt=prompt, template=sfc_template,
                    label_word_list=label_word_list, verbose=verbose
                )
                features = OurInputFeatures(**inputs, label=example_label, sfc_input_ids=sfc_feature.input_ids,
                                            sfc_attention_mask=sfc_feature.attention_mask,
                                            sfc_mask_pos=sfc_feature.mask_pos)
            else:
                features = OurInputFeatures(**inputs, label=example_label)
